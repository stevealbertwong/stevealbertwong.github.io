...
\\(X^T X = V {\Sigma}^2 V^T\\) is diagonalization. Gemetrically, it corresponds to rotate, stretches and rotates back and in that new coordinate system variance is maximized.
...
Focusing on \\(\det(X^T X- \Sigma^2 I) = 0\\), somehow rotating the covariance matrix is maximizing the variance in new coordinate system. 

Finding solution to lambda i.e. linear dependence in covariance matrix rotation is  a contraint maximzation question asking what is the biggest lambda could be? the contraint is covaraicne matrix each column vector could only rotate in 1 direction (recall \\(\det(X^T X- \Sigma^2 I) = 0\\))
...
<div class="imgcap">
<img src="/assets/svd/squish1.png" height="200" style="float: left; width: 49%; margin-right: 1%; margin-bottom: 0.5em;">

<div class="imgcap">
<img src="/assets/svd/squish2.png" height="200" style="float: right; width: 49%; margin-right: 1%; margin-bottom: 0.5em;">
</div>
</div>
...
Why diagonalization implies arg max variance?

diagonalization as rotate, stretch, rotate back => direction of maximum stretch after rotate into new coordinate system => maximize variance => diagonal in disguise 


what should lambda be to bring column vectors in transformation matrix to be linearly dependent (i.e. squish eigenvector to 0)
...
[pagerank random surfer](http://www.math.cornell.edu/~mec/Winter2009/RalucaRemus/Lecture3/lecture3.html)
