...
1st step already does most of the heavy lifting.
2nd step is rather strightforward, just solve U since we have already found V and \\(\Sigma \\). The interesting part of 2nd step is geometrically computing \\(XV\\) is actually computing the distance between the new coordinate system and data points. To see this, we need our friend projection onto subspace.
...
$$ \sum_{i} x^2 = (proj_v(x))^2 + (dist_v(x))^2 $$


linear map that projects x onto v:
$$ proj_v: \mathbb{R} \to \mathbb{R} $$


dot product: v scaled by inner product of x and v:
$$ proj_v(x): \mathbb{R} \to \mathbb{R} $$


$$ proj_v(x) = (x.v)v $$


compute square length of projection by taking dot product: \\(|x \cdot y|\\)

distance of \\(x \\) from line spanned by unit vector \\(v\\):

$$ (dist_v(x))^2 = (\sum_{i=1}^{n} {x_i}^2) - |x \cdot v| $$ 


\\(Av\\) is \\(x_i \dot v\\) at each entry

squared norm of \\(|Av|^2\\): sum of squred lengths of projections of data points onto line spanned by v


$$ \DeclareMathOperator*{\argmax}{arg\,max} v_2 = \argmax_{v \bot v_1, ||v|| =1} $$

$$ \DeclareMathOperator*{\argmax}{arg\,max} \argmax_{v:||v||=1} |Av|$$


**Maximizing squared projection is the same as minimizing squared Euclidean distance between projected line and data points**
