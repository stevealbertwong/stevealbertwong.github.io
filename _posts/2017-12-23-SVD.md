...
But why does rotating covariance matrix (diagonalization) implies \\(\DeclareMathOperator*{\argmax}{arg\,max} \argmax_\\) variance in rotating tranformation matrix (\\(XV = U\Sigma\\))? (try to picture 2 rotations simultaneously side by side)
