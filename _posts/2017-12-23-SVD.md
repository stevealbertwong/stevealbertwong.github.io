...
\\(dist_v(x)\\) denotes distance of \\(x \\) from line spanned by unit vector \\(v\\)
...
**Maximizing sum of squared lengths of projections is the same as minimizing squared Euclidean distance between projected line and data points**, as squared length of x is constant.
