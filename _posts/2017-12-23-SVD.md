...
\\(XV\\) is projection length of all data points \\(x_i\\) onto 3 new coordinate lines spanned by \\(V\\). Each entry is dot products \\( x_i \cdot v_i \\). The squared norm \\( {||XV||}^2 \\) denotes sum of squared lengths of projections of data onto line spanned by \\(v\\)
