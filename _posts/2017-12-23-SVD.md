...
\\( proj_v(x): \mathbb{R} \to \mathbb{R} \\): \\( proj_v(x)\\) linear maps input vector \\(x\\) and output projection of \\(x\\) onto \\(v\\) 


\\( proj_v(x)\\) is computed by v scaled by inner product of x and v
